...
import { z } from "zod";
var loaded = dotenv.config();
if (loaded.error && loaded.error.code !== "ENOENT") {
  throw loaded.error;
}
var envSchema = z.object({
  NODE_ENV: z.enum(["development", "test", "production"]).default("production"),
  DATABASE_URL: z.string({ required_error: "DATABASE_URL is required" }).min(1, "DATABASE_URL must be set").refine(
...
if (parsed.data.ADMIN_BOOTSTRAP_EMAIL && !parsed.data.ADMIN_BOOTSTRAP_PASSWORD || !parsed.data.ADMIN_BOOTSTRAP_EMAIL && parsed.data.ADMIN_BOOTSTRAP_PASSWORD) {
  throw new Error("Both ADMIN_BOOTSTRAP_EMAIL and ADMIN_BOOTSTRAP_PASSWORD must be set together.");
}
var port = parsed.data.PORT === void 0 || parsed.data.PORT === "" ? 5e3 : Number.parseInt(parsed.data.PORT, 10);
...
function setupCors(app2) {
  app2.use((req, res, next) => {
    ...
    if (origin) {
      ...
      const isAllowed = allowedPatterns.some((pattern) => pattern.test(origin)) || env.replitDevDomain && origin.includes(env.replitDevDomain);
      if (isAllowed) {
...
async function ensureBootstrapAdmin() {
  ...
  try {
    ...
    if (!existingUser) {
      ...
      log(`[bootstrap] Created admin user ${email}`);
      return;
    }
    if (!existingUser.isActive || existingUser.password !== hashedPassword || existingUser.role?.toUpperCase() !== "ADMIN") {
      await storage.updateUser(existingUser.id, {
        isActive: true,
        password: hashedPassword,
        role: "ADMIN"
      });
      log(`[bootstrap] Ensured admin user ${email} is active with configured credentials`);
    } else {
      log(`[bootstrap] Admin user ${email} already active`);
    }
  } catch (error) {
    console.error(`[bootstrap] Failed to ensure admin user ${email}:`, error);
  }
}
...
(async () => {
  ...
  server.listen(
    ...
    () => {
      log(`express server serving on port ${port2}`);
    }
  );
})();
